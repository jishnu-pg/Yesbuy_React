...
import { getTermsAndConditions } from "../services/api/terms";
  ...
  const fetchTermsAndConditions = async () => {
    try {
      ...
      if (response.results && response.results.length > 0) {
        const termsData = response.results[0];
    ...
    return (
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <LoaderSpinner label="Loading Terms & Conditions..." />
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {content ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <p className="text-gray-600">No Terms & Conditions available at the moment.</p>
          </div>
        )}
      </div>
    </div>
